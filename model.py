...
class Gate(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            log_probs_and_noise = torch.log(logit_probabilities) + gumbel_noise
            final_probabilities = F.softmax(
                log_probs_and_noise / self.temperature, dim=1
            )
        else:
...
class EmotionClassifier(nn.Module):
    ...
    def forward(self, x):
        x = self.conformer(x)
        x = x.mean(dim=1)  # could do max here, could add the standard deviation
